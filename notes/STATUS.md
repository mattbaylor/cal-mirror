...
| 20a | ~~**Send times · personal links · zero-decision setup**~~ — **decided 16 Sept: all three are in 2.0**, after the native pass, in that order. `decisions.md`, *Settled*. **All three built 16 Sept** (#108, #109, and the setup PR). |
...
- **`CM_RELEASE_TOKEN`** lets `watch-review.yml` stamp the site on `main` when a version clears review. Fine-grained PAT on `mattbaylor`. **Died in the 17 Sept secrets rotation** (the runbook thought it was a different token); the watcher failed every three hours from then until a new PAT was set. The workflow now checks the token each run and says so in plain words when it is rejected, instead of dying at checkout. Its fallback (open a PR) is refused by a repo setting, so renew it or turn *Allow GitHub Actions to create PRs* on before it expires. The push path has still never been exercised by a real release.
