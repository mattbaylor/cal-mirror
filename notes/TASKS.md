...
| 7 | **Rejected 22 Sept, Guideline 2.1(b)** — App Review could not find the three subscriptions in the binary. Fixed in code: all three are buyable on the offer screen (`decisions.md`, *Proposed*, *All three tiers are buyable on the offer screen*), and the review notes walk from install to that screen step by step. Also fixed, before the build: the two upgrade rows showed a bare "$34.99" with no period, which is what 3.1.2 asks for on a subscription screen. Left: **build 13** from `release.yml` (`platforms: ios` — Mac 2.0 is live at 12); `asc-apply` for the notes; resubmit submission `3d11811f` from the App Review page — the three subscriptions and the group are still `READY_FOR_REVIEW` on it and must **not** be removed and re-added; a sandbox purchase of each tier on a physical device, screen-recorded from the Home Screen, attached in the App Review notes. `AW_APPSTORE_SANDBOX` must stay 1 until approval. | **Matt** |
| 8 | **The real 2.1(b) cause, found 23 Sept from Matt's own screen recording** — the offer screen's automatic product load cancelled itself (`load()` set `.loading`, which flipped the `.task(id:)` id), so it had never worked on a device: "Could not reach the App Store" in under a second, and "Try again" always fixed it. Fixed with a plain `.task` plus cancellation checks in the retry loop, build **14**. Consequence still open: **macOS 2.0 is live with this bug** — every Mac customer's first look at AskWhen.me is the failure screen once the subscriptions go live. A live version's build cannot be replaced, so the Mac fix needs a **2.0.1**. | **Matt** to decide when |
| 9 | **The subdomain is asked for before it is bought (24 Sept)** — Matt: "they have to buy before they know if they can get their desired subdomain? I don't like that." Now a name field and *Check it* under the Custom Subdomain row on the offer screen; `GET /v1/subdomains/{label}` answers, `POST /v1/subdomains/{label}/hold` reserves for 15 minutes on Subscribe, and a name lost in between means nothing is bought. Build **15**. The offer screen's privacy footer is dropped rather than reworded (Matt's call, 24 Sept): the ordering it described still holds, it is just no longer printed. Hold and check now have separate rate budgets — 60 checks an hour, 5 holds — and `/hold` is gated on a key baked into the build (`X-Askwhen-App`). **Two things must match before build 15 ships:** `openssl rand -hex 16 > askwhen/infra/secrets/hold_keys` on CT 112, and the same value as the `AW_HOLD_KEY` GitHub secret. A build whose key is not in that file cannot reserve names; an empty file turns the gate off and the service warns at startup. | **Matt** to read |
...
| **Terms for the request page** — drafted as `site/terms.html` (16 Sept): the tiers, the grace week, a released subdomain may be claimed by someone else, a domain stays yours and only the pointer goes, what is promised and what is not. Matt approves. | Needed before the domain tier is sold. |
