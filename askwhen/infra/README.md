...
# Postal API key for the AskWhen.me organization's `main` server, type API.
# Created in Postal (dlvr.rehosted.us) and held in Infisical as
# postal_api_key; this is a copy of it, not the source of truth. Copy it
# without it passing through a terminal you can scroll back:
#   infisical run --env=prod -- sh -c 'printf %s "$postal_api_key"' \
#     | ssh root@172.16.1.41 'cat > /opt/cal-mirror/askwhen/infra/secrets/postal_api_key'

# The token pepper. Generate it here, once, and never again — see below.
head -c 32 /dev/urandom | base64 > secrets/pepper

# The on-demand TLS authorization secret. The same value goes into the edge
# Caddyfile's `ask` URL (edge.md); it can be rotated, the pepper cannot.
openssl rand -hex 32 > secrets/tls_auth_secret

# The app build key for reserving a subdomain name. THE SAME VALUE must be the
# AW_HOLD_KEY GitHub secret, which release.yml bakes into the build; a build
# whose key is not in this file cannot reserve names, and an empty file turns
# the gate off entirely (the service warns at startup when it is).
# Comma-separate during a rotation: new key first, drop the old one once that
# build is gone from devices.
#   openssl rand -hex 16 > secrets/hold_keys
#   gh secret set AW_HOLD_KEY < secrets/hold_keys     # run where gh is mattbaylor
openssl rand -hex 16 > secrets/hold_keys
...
|---|---|---|---|
| `secrets/postal_api_key` | app host, mounted read-only into the container | Postal on `dlvr.rehosted.us`, the AskWhen.me organization's `main` server; copy in Infisical | The holder can send mail as askwhen.me. Revoke in Postal, issue a new one, rewrite the file, `up --apply`. |
...
| `secrets/tls_auth_secret` | app host, and the edge Caddyfile's `ask` URL | `openssl rand -hex 32` | The holder can get the edge to issue certificates for names they point at it — spending the Let's Encrypt budget the other sites renew from. Rotate in both places; nothing else depends on it. |
| `secrets/hold_keys` | app host, and the `AW_HOLD_KEY` GitHub secret | `openssl rand -hex 16`, matched in both places | Very little, and it is half-leaked by design: it ships inside the app, and anyone who downloads it can read the value out of the bundle in minutes. It is a floor under `POST /v1/subdomains/{label}/hold`, not a credential — someone holding it can reserve names at 5/hour/IP for 15 minutes each, and reserving grants nothing without a paid subscription. Rotate by shipping a new build and dropping the old key once that build is gone. **It is in a file rather than `compose.yml` only because this repository is public**, and a key anyone can read here is no floor at all. |
| DKIM private key | **`dlvr.rehosted.us` only** | generated by Postal when the sending domain was added | The holder can forge signed mail as askwhen.me. Rotate in Postal, which publishes a new selector; update the DNS record. |

The DKIM key is the reason the service submits mail rather than sending it. The
app container parses untrusted input from a public form; it holds a key that can
*send* mail and nothing that can *forge* it. That separation is worth a network
hop.
...
2. **`GET /c/{confirm_token}` is a mutating GET.** Outlook's Safe Links, Gmail's
   ...
   the human sees it, which turns double opt-in — the entire spam defense — into
...
3. **Hostname-to-slug mapping.** §7 sells "several pages" from $35 and a custom
   ...
   authorize against; that is a placeholder for a decision, not the decision.
