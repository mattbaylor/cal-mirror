...
# Postal API key for the askwhen.me organisation's `main` server, type API.
...
# The on-demand TLS authorisation secret. The same value goes into the edge
...
|---|---|---|---|
| `secrets/postal_api_key` | app host, mounted read-only into the container | Postal on `dlvr.rehosted.us`, the askwhen.me organisation's `main` server; copy in Infisical | The holder can send mail as askwhen.me. Revoke in Postal, issue a new one, rewrite the file, `up --apply`. |
