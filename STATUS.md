...
| | What | Whose | Size |
|---|---|---|---|
| 1 | ~~The request-page UI in the app~~ **done 15 Sept** ([#99](https://github.com/mattbaylor/cal-mirror/pull/99)). Fifteen screens, approved against `apple/tools/review.html`. **Never run** — a simulator pass is the first time a person sees it move, and belongs with 9. | done, unproven | — |
| 2 | ~~The purchase UI~~ **done 15 Sept**, in the same PR. The offer screen sells the Request Page trial; subdomain and domain are sold on the address screen, where the want appears. Restore Purchases is there because Apple requires it. **Never run against StoreKit** — the `.storekit` file is attached to both run schemes, so a debug build exercises it with no network and no sandbox account. | done, unproven | — |
