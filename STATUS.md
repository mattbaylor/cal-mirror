...
| 2 | **StoreKit in the app.** App Store Connect is done (15 Sept: group, three products, trial on Page only, notification URLs, sandbox tester, `.storekit` file). What remains is the purchase UI and the Kit wrapper that hands `Transaction.jwsRepresentation` to `create`. | **Yours** (UI) — the wrapper is mine | days |
