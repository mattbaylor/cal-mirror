...
| A5 | ~~**The write token cannot be recovered.**~~ Fixed 16 Sept: `kSecAttrSynchronizable`, migration of pre-2.0 items, and a *Carry on* row on a device that has the key and no config. | `KeychainTokenStore` | done |
