...
for p in DOCS.rglob("*.html"):
    ...
    if QUALIFIER.search(text):
        problems.append(f"{p.relative_to(DOCS)}: a link label is qualified with a language")
...
def url_for(rel: Path, dirname: str) -> str:
    ...
    if s.endswith("index.html"):
        s = s[: -len("index.html")]
    return SITE + dirname + s

for rel in sorted(english):
    for code, dirname in LANGS:
        page = DOCS / dirname / rel
        if not page.exists():
            continue
        text = page.read_text()
        for other, otherdir in LANGS:
            ...
            if want not in text:
                problems.append(f"{page.relative_to(DOCS)}: missing alternate {want}")
        if f'hreflang="x-default" href="{url_for(rel, "")}"' not in text:
            problems.append(f"{page.relative_to(DOCS)}: missing x-default alternate")

# --- 5. the sitemap lists everything ----------------------------------------
sitemap = (DOCS / "sitemap.xml").read_text()
for rel in sorted(english):
    for code, dirname in LANGS:
        if not (DOCS / dirname / rel).exists():
            continue
